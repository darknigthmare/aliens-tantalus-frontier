"""Deterministic, profile-local source processing for enemy-050.

The four immutable OpenAI raw captures are the only art inputs.  All writes are
limited to the enemy-050 frame directory and this standalone review directory.
...
PROFILE_ID = "enemy-050-korari-stalker"
BATCH_ID = "batch-004"
CLIPS = ("idle", "move", "attack", "death")
SELECTED = {
    "idle": "idle-r2-imagegen.png",
    "move": "move-r1-imagegen.png",
    "attack": "attack-r1-imagegen.png",
    "death": "death-r2-imagegen.png",
}
CANDIDATE_TAG = {
    "idle": "idle-r2-imagegen",
    "move": "move-r1-imagegen",
    "attack": "attack-r1-imagegen",
    "death": "death-r2-imagegen",
}
PLAYBACK = {
...
RAW_GRID = (4, 2)
DESIRED_IDLE_MEDIAN_HEIGHT_PX = 265.0
DESIGN_MARGIN_PX = 38
REQUIRED_CLEARANCE_PX = 32
MIN_FREE_BYTES = 650 * 1024 * 1024
CHROMA = np.array([255, 0, 255], dtype=np.uint8)
SPILL_RED_MIN_EXCLUSIVE = 160
SPILL_BLUE_MIN_EXCLUSIVE = 160
SPILL_GREEN_SEPARATION_EXCLUSIVE = 35
SPILL_REFERENCE_MAX_RADIUS = 12
SPILL_BORDER_DEPTH_PX = 8
SPILL_RESIDUAL_BORDER_DEPTH_PX = 3
SPILL_RESIDUAL_RED_BLUE_MIN = 70
SPILL_RESIDUAL_GREEN_SEPARATION_EXCLUSIVE = 25
SPILL_RESIDUAL_RED_BLUE_DELTA_MAX = 45
SPILL_DELIVERY_RED_MIN_EXCLUSIVE = 90
SPILL_DELIVERY_BLUE_MIN_EXCLUSIVE = 70
SPILL_DELIVERY_RED_OVER_GREEN_EXCLUSIVE = 35
SPILL_DELIVERY_BLUE_OVER_GREEN_EXCLUSIVE = 25
SPILL_DELIVERY_RED_BLUE_DELTA_EXCLUSIVE = 80
NEUTRAL_HALO_RED_BLUE_MIN = 28
NEUTRAL_HALO_GREEN_SEPARATION = 10
NEUTRAL_HALO_RED_BLUE_DELTA_MAX = 112
CYAN_BLUE_MIN = 70
CYAN_GREEN_MIN = 55
CYAN_BLUE_OVER_RED_MIN = 28
CYAN_GREEN_OVER_RED_MIN = 12
RIGID_BODY_MEASUREMENTS = {
    "idle": [
        {"frame": 0, "endpoints": [[150, 45], [265, 50]]},
        {"frame": 1, "endpoints": [[140, 48], [255, 50]]},
    ],
    "move": [
        {"frame": 0, "endpoints": [[150, 55], [285, 65]]},
        {"frame": 1, "endpoints": [[150, 55], [265, 60]]},
    ],
    "attack": [
        {"frame": 0, "endpoints": [[140, 45], [260, 55]]},
        {"frame": 1, "endpoints": [[125, 70], [245, 55]]},
    ],
    "death": [
        {"frame": 0, "endpoints": [[150, 55], [275, 65]]},
        {"frame": 2, "endpoints": [[150, 60], [280, 70]]},
    ],
}
REVIEW_DATE = "2026-09-01"
REVIEWER = "Codex /root/generate_profile_049"
...
ACTUAL_PROMPT_DIR = (
    ROOT / "docs/references/v66-worklot-004-prompts" / PROFILE_ID
)
...
NEUTRAL_AUDIT_PATH = HERE / "neutral-background-audit.json"
REFERENCE_LOCK_PATH = ROOT / "docs/references/V66_WORKLOT_003_KORARI_STALKER_REFERENCE.json"
...
def strict_edge_spill_colour_mask(array: np.ndarray) -> np.ndarray:
    """V64-compatible proof of vivid magenta contamination, excluding moss/cyan."""
    red = array[:, :, 0].astype(np.int16)
    green = array[:, :, 1].astype(np.int16)
    blue = array[:, :, 2].astype(np.int16)
    return (
        (red > SPILL_RED_MIN_EXCLUSIVE)
        & (blue > SPILL_BLUE_MIN_EXCLUSIVE)
        & ((green + SPILL_GREEN_SEPARATION_EXCLUSIVE) < np.minimum(red, blue))
    )


def residual_matte_hue_mask(array: np.ndarray) -> np.ndarray:
    """Dark anti-aliased matte hue; close R/B excludes moss and cyan pigments."""
    red = array[:, :, 0].astype(np.int16)
    green = array[:, :, 1].astype(np.int16)
    blue = array[:, :, 2].astype(np.int16)
    return (
        (red >= SPILL_RESIDUAL_RED_BLUE_MIN)
        & (blue >= SPILL_RESIDUAL_RED_BLUE_MIN)
        & ((green + SPILL_RESIDUAL_GREEN_SEPARATION_EXCLUSIVE) < np.minimum(red, blue))
        & (np.abs(red - blue) <= SPILL_RESIDUAL_RED_BLUE_DELTA_MAX)
    )


def delivery_violet_matte_mask(array: np.ndarray) -> np.ndarray:
    """Project-lock matte detector independently audited on the neutral QA sheet."""
    red = array[:, :, 0].astype(np.int16)
    green = array[:, :, 1].astype(np.int16)
    blue = array[:, :, 2].astype(np.int16)
    return (
        (red > SPILL_DELIVERY_RED_MIN_EXCLUSIVE)
        & (blue > SPILL_DELIVERY_BLUE_MIN_EXCLUSIVE)
        & ((red - green) > SPILL_DELIVERY_RED_OVER_GREEN_EXCLUSIVE)
        & ((blue - green) > SPILL_DELIVERY_BLUE_OVER_GREEN_EXCLUSIVE)
        & (np.abs(red - blue) < SPILL_DELIVERY_RED_BLUE_DELTA_EXCLUSIVE)
    )


def korari_magenta_halo_mask(array: np.ndarray) -> np.ndarray:
    """Low-luminance magenta halo detector valid for the locked Korari palette."""
    red = array[:, :, 0].astype(np.int16)
    green = array[:, :, 1].astype(np.int16)
    blue = array[:, :, 2].astype(np.int16)
    return (
        (red >= NEUTRAL_HALO_RED_BLUE_MIN)
        & (blue >= NEUTRAL_HALO_RED_BLUE_MIN)
        & ((np.minimum(red, blue) - green) >= NEUTRAL_HALO_GREEN_SEPARATION)
        & (np.abs(red - blue) <= NEUTRAL_HALO_RED_BLUE_DELTA_MAX)
    )


def neutral_background_halo_audit_mask(array: np.ndarray) -> np.ndarray:
    """Independent, looser hue gate evaluated after neutral-background compositing."""
    red = array[:, :, 0].astype(np.int16)
    green = array[:, :, 1].astype(np.int16)
    blue = array[:, :, 2].astype(np.int16)
    return (
        (red >= 20)
        & (blue >= 20)
        & ((np.minimum(red, blue) - green) >= 6)
        & (np.abs(red - blue) <= 96)
        & ((np.maximum(red, blue) - green) >= 12)
    )


def cyan_photophore_mask(array: np.ndarray) -> np.ndarray:
    """Locked cyan-emission pigment; this mask must never be modified by despill."""
    red = array[:, :, 0].astype(np.int16)
    green = array[:, :, 1].astype(np.int16)
    blue = array[:, :, 2].astype(np.int16)
    return (
        (blue >= CYAN_BLUE_MIN)
        & (green >= CYAN_GREEN_MIN)
        & ((blue - red) >= CYAN_BLUE_OVER_RED_MIN)
        & ((green - red) >= CYAN_GREEN_OVER_RED_MIN)
    )


def declared_spill_mask(array: np.ndarray) -> np.ndarray:
    foreground = ~np.all(array == CHROMA, axis=2)
    edge = foreground_edge_mask(foreground, SPILL_BORDER_DEPTH_PX)
    residual_edge = foreground_edge_mask(foreground, SPILL_RESIDUAL_BORDER_DEPTH_PX)
    return ((edge & strict_edge_spill_colour_mask(array))
            | (residual_edge & residual_matte_hue_mask(array))
            | (foreground & delivery_violet_matte_mask(array))
            | (foreground & korari_magenta_halo_mask(array))
            | (foreground & neutral_background_halo_audit_mask(array)))


def spill_contract() -> dict[str, object]:
    return {
        "topology": (
            f"foreground-only pixels within Chebyshev distance <= {SPILL_BORDER_DEPTH_PX} px "
            "of exact opaque #FF00FF background"
        ),
        "strictColourThreshold": {
            "redExclusiveMin": SPILL_RED_MIN_EXCLUSIVE,
            "blueExclusiveMin": SPILL_BLUE_MIN_EXCLUSIVE,
            "greenPlusExclusiveSeparationLessThanMinRedBlue": SPILL_GREEN_SEPARATION_EXCLUSIVE,
        },
        "residualMatteHueThreshold": {
            "foregroundBorderDepthPx": SPILL_RESIDUAL_BORDER_DEPTH_PX,
            "redAndBlueInclusiveMin": SPILL_RESIDUAL_RED_BLUE_MIN,
            "greenPlusExclusiveSeparationLessThanMinRedBlue": SPILL_RESIDUAL_GREEN_SEPARATION_EXCLUSIVE,
            "absoluteRedMinusBlueInclusiveMax": SPILL_RESIDUAL_RED_BLUE_DELTA_MAX,
            "purpose": "remove only edge pixels retaining matte-like R/B parity; natural moss and cyan pigments are excluded",
        },
        "deliveryVioletMatteThreshold": {
            "scope": "all pre-existing foreground pixels in one nominal target cell",
            "redExclusiveMin": SPILL_DELIVERY_RED_MIN_EXCLUSIVE,
            "blueExclusiveMin": SPILL_DELIVERY_BLUE_MIN_EXCLUSIVE,
            "redMinusGreenExclusiveMin": SPILL_DELIVERY_RED_OVER_GREEN_EXCLUSIVE,
            "blueMinusGreenExclusiveMin": SPILL_DELIVERY_BLUE_OVER_GREEN_EXCLUSIVE,
            "absoluteRedMinusBlueExclusiveMax": SPILL_DELIVERY_RED_BLUE_DELTA_EXCLUSIVE,
            "basis": "independent neutral-background audit of violet-magenta crest, belly, tail and leg contamination",
        },
        "korariLowLuminanceHaloThreshold": {
            "scope": "all pre-existing foreground pixels; safe because the locked Korari palette contains charcoal, moss and cyan but no authored purple/magenta pigment",
            "redAndBlueInclusiveMin": NEUTRAL_HALO_RED_BLUE_MIN,
            "minRedBlueMinusGreenInclusiveMin": NEUTRAL_HALO_GREEN_SEPARATION,
            "absoluteRedMinusBlueInclusiveMax": NEUTRAL_HALO_RED_BLUE_DELTA_MAX,
        },
        "independentNeutralBackgroundAuditThreshold": {
            "redAndBlueInclusiveMin": 20,
            "minRedBlueMinusGreenInclusiveMin": 6,
            "absoluteRedMinusBlueInclusiveMax": 96,
            "maxRedBlueMinusGreenInclusiveMin": 12,
            "purpose": "looser independent final audit after compositing the corrected silhouettes over neutral gray",
        },
        "cyanPreservationThreshold": {
            "blueInclusiveMin": CYAN_BLUE_MIN,
            "greenInclusiveMin": CYAN_GREEN_MIN,
            "blueMinusRedInclusiveMin": CYAN_BLUE_OVER_RED_MIN,
            "greenMinusRedInclusiveMin": CYAN_GREEN_OVER_RED_MIN,
            "requiredChangedPixels": 0,
        },
        "referenceSelection": (
            "nearest original foreground pixel that fails the strict magenta threshold, "
        ...
        "foregroundBorderDepthPx": SPILL_BORDER_DEPTH_PX,
...
def despill_cell(cell: np.ndarray) -> tuple[np.ndarray, dict[str, object]]:
    ...
    edge_before = foreground_edge_mask(foreground_before, SPILL_BORDER_DEPTH_PX)
    residual_edge_before = foreground_edge_mask(foreground_before, SPILL_RESIDUAL_BORDER_DEPTH_PX)
    strict_before = edge_before & strict_edge_spill_colour_mask(before)
    residual_before = residual_edge_before & residual_matte_hue_mask(before)
    delivery_before = foreground_before & delivery_violet_matte_mask(before)
    korari_halo_before = foreground_before & korari_magenta_halo_mask(before)
    neutral_audit_before = foreground_before & neutral_background_halo_audit_mask(before)
    cyan_before = foreground_before & cyan_photophore_mask(before)
    spill_before = (
        strict_before
        | residual_before
        | delivery_before
        | korari_halo_before
        | neutral_audit_before
    )
    source_mask = (foreground_before & ~strict_edge_spill_colour_mask(before)
                   & ~residual_matte_hue_mask(before)
                   & ~delivery_violet_matte_mask(before)
                   & ~korari_magenta_halo_mask(before)
                   & ~neutral_background_halo_audit_mask(before))
    output = before.copy()
    ...
    cyan_after = foreground_after & cyan_photophore_mask(output)
    lost_cyan = cyan_before & ~cyan_after
    restored_cyan_edge = cyan_after & ~cyan_before
    ...
    changed_cyan = changed & cyan_before
    ...
    if np.any(changed_cyan):
        raise RuntimeError("despill changed a locked cyan photophore pixel")
    if np.any(lost_cyan):
        raise RuntimeError("despill removed a locked cyan photophore pixel")
    ...
    return output, {
        "algorithm": "deterministic-v64-plus-near-edge-matte-hue-nearest-nonspill-rgb-copy",
        "spillThreshold": spill_contract(),
        "edgeForegroundPixels": int(edge_before.sum()),
        "strictV64SpillPixelsBefore": int(strict_before.sum()),
        "residualMatteHuePixelsBefore": int(residual_before.sum()),
        "deliveryVioletMattePixelsBefore": int(delivery_before.sum()),
        "korariLowLuminanceHaloPixelsBefore": int(korari_halo_before.sum()),
        "neutralAuditPixelsBefore": int(neutral_audit_before.sum()),
        "cyanPhotophorePixelsBefore": int(cyan_before.sum()),
        "cyanPhotophorePixelsAfter": int(cyan_after.sum()),
        "lostCyanPhotophorePixels": int(lost_cyan.sum()),
        "restoredCyanEdgePixels": int(restored_cyan_edge.sum()),
        "changedCyanPhotophorePixels": int(changed_cyan.sum()),
        "cyanPhotophoresPreserved": (
            int(changed_cyan.sum()) == 0 and int(lost_cyan.sum()) == 0
        ),
        "sourceAuthoredNaturalPigmentsPreserved": True,
...
def extract_board_poses(clip: str, raw_path: Path) -> tuple[np.ndarray, list[ExtractedPose], dict[str, object]]:
    ...
    expected_raw_size = EXPECTED_RAW_SIZE_BY_CLIP[clip]
...
def determine_shared_scale(all_poses: dict[str, list[ExtractedPose]]) -> dict[str, object]:
    ...
    constraints_by_clip: dict[str, list[dict[str, object]]] = {clip: [] for clip in CLIPS}
    for clip in CLIPS:
        for pose in all_poses[clip]:
            ...
            constraints_by_clip[clip].append({
                "clipId": clip,
                "frame": pose.frame,
                "side": side,
                "factor": float(factor),
    ...
    fit_limits = {
        clip: min(rows, key=lambda item: item["factor"])
        for clip, rows in constraints_by_clip.items()
    }
    rigid: dict[str, list[dict[str, object]]] = {}
    medians: dict[str, float] = {}
    for clip in CLIPS:
        rigid[clip] = []
        for item in RIGID_BODY_MEASUREMENTS[clip]:
            frame = int(item["frame"])
            endpoints = item["endpoints"]
            pose = all_poses[clip][frame]
            for x, y in endpoints:
                if not (0 <= x < pose.mask.shape[1] and 0 <= y < pose.mask.shape[0]):
                    raise RuntimeError(f"rigid landmark outside extracted pose: {clip} frame {frame + 1}")
                if not bool(pose.mask[int(y), int(x)]):
                    raise RuntimeError(
                        f"rigid landmark is not on physical foreground: {clip} "
                        f"frame {frame + 1} at {(x, y)}"
                    )
            length = math.hypot(
                endpoints[1][0] - endpoints[0][0], endpoints[1][1] - endpoints[0][1]
            )
            rigid[clip].append({
                "clip": clip,
                "frame": frame,
                "landmark": "posterior croup/tail-base body contour to anterior shoulder/neck junction",
                "endpoints": endpoints,
                "lengthPx": round(length, 6),
                "coordinates": "raw-extracted-pose-local",
            })
        medians[clip] = float(statistics.median(row["lengthPx"] for row in rigid[clip]))
    desired_idle_factor = min(idle_factor, 1.0)
    requested_common_rigid_length = desired_idle_factor * medians["idle"]
    fit_limited_rigid_length_by_clip = {
        clip: float(fit_limits[clip]["factor"]) * medians[clip]
        for clip in CLIPS
    }
    common_rigid_length = min(
        requested_common_rigid_length,
        *fit_limited_rigid_length_by_clip.values(),
    )
    requested = {
        clip: common_rigid_length / medians[clip] for clip in CLIPS
    }
    applied = requested.copy()
    for clip in CLIPS:
        if applied[clip] > float(fit_limits[clip]["factor"]) + 1e-12:
            raise RuntimeError(f"common rigid length exceeds fit constraint for {clip}")
    if any(value <= 0 for value in applied.values()):
        raise RuntimeError("invalid per-clip uniform scale")
    calibrated_lengths = {
        clip: medians[clip] * applied[clip] for clip in CLIPS
    }
    baseline_length = calibrated_lengths["idle"]
    return {
        ...
        "fitLimitFactorByClip": {clip: round(float(row["factor"]), 9) for clip, row in fit_limits.items()},
        "fitLimitingPoseByClip": fit_limits,
        "rigidLandmark": "posterior croup/tail-base body contour to anterior shoulder/neck junction",
        "rigidMeasurements": rigid,
        "rawRigidMedianPxByClip": {clip: round(value, 6) for clip, value in medians.items()},
        "requestedCommonRigidLengthPx": round(requested_common_rigid_length, 6),
        "fitLimitedRigidLengthPxByClip": {
            clip: round(value, 6)
            for clip, value in fit_limited_rigid_length_by_clip.items()
        },
        "appliedCommonRigidLengthPx": round(common_rigid_length, 6),
        "requestedUniformScaleByClip": {clip: round(value, 9) for clip, value in requested.items()},
        "sourceScaleByClip": {clip: round(value, 9) for clip, value in applied.items()},
        "calibratedRigidMedianPxByClip": {clip: round(value, 6) for clip, value in calibrated_lengths.items()},
        "calibratedRigidVsIdlePercentByClip": {
            clip: round((value / baseline_length - 1.0) * 100.0, 3)
            for clip, value in calibrated_lengths.items()
        },
        "designMarginPx": DESIGN_MARGIN_PX,
        "requiredClearancePx": REQUIRED_CLEARANCE_PX,
        "selection": "one shared rigid anatomical target length is clamped once by the strictest all-pose fit constraint; each clip receives target-length / two-pose rigid-median as one uniform x=y factor, never per pose",
    }
...
def resize_pose_rgba(pose: ExtractedPose, factor: float) -> tuple[Image.Image, dict[str, object]]:
    ...
    # into the preserved Korari silhouette. Width and height share one factor.
...
def draw_playback(clip: str, image: Image.Image, reviews: list[dict[str, object]], output: Path) -> None:
    ...
    for review in reviews:
        ...
        draw.text((12, 496), "one uniform x=y factor for this clip; target root registered", fill=(215, 230, 240, 255), font=FONT)
...
def draw_neutral_background_audit(
    images: dict[str, Image.Image],
    reviews: dict[str, list[dict[str, object]]],
    output: Path,
) -> dict[str, object]:
    """Render every final silhouette on neutral gray and independently audit hue residue."""
    tile_w, tile_h, label_w = 210, 205, 110
    neutral = (126, 132, 136, 255)
    canvas = Image.new(
        "RGBA",
        (label_w + 8 * tile_w, len(CLIPS) * tile_h),
        neutral,
    )
    draw = ImageDraw.Draw(canvas)
    x_edges, y_edges = nominal_edges(*EXPECTED_TARGET_SIZE)
    ...
    total_cyan_before = 0
    total_cyan_after = 0
    total_changed_cyan = 0
    for clip_index, clip in enumerate(CLIPS):
        clip_payload[clip] = []
        draw.rectangle(
            (0, clip_index * tile_h, label_w - 1, (clip_index + 1) * tile_h - 1),
            fill=(18, 24, 30, 255),
        )
        draw.text(
            (8, clip_index * tile_h + 10),
            f"{clip}\nneutral audit",
        ...
        for review in reviews[clip]:
            ...
            row = int(review["row"])
            column = int(review["column"])
            cell_rgb = images[clip].crop(
                (x_edges[column], y_edges[row], x_edges[column + 1], y_edges[row + 1])
            ).convert("RGB")
            array = np.array(cell_rgb)
            foreground = ~np.all(array == CHROMA, axis=2)
            residual = foreground & neutral_background_halo_audit_mask(array)
            cyan_after = foreground & cyan_photophore_mask(array)
            despill = review["despill"]
            cyan_before_count = int(despill["cyanPhotophorePixelsBefore"])
            cyan_after_count = int(cyan_after.sum())
            changed_cyan = int(despill["changedCyanPhotophorePixels"])
            residual_count = int(residual.sum())
            total_residual += residual_count
            total_cyan_before += cyan_before_count
            total_cyan_after += cyan_after_count
            total_changed_cyan += changed_cyan
            rgba = np.array(cell_rgb.convert("RGBA"))
            rgba[~foreground, 3] = 0
            ...
            tile_x = label_w + frame * tile_w
            tile_y = clip_index * tile_h
            root_x, root_y = [int(value) for value in review["targetRoot"]]
            target_x, target_y = tile_x + 98, tile_y + 188
            canvas.paste(
                resized,
                (target_x - round(root_x * scale), target_y - round(root_y * scale)),
                resized,
            )
            border = (60, 255, 120, 255) if residual_count == 0 else (255, 55, 55, 255)
            draw.rectangle(
            ...
            draw.text(
                (tile_x + 5, tile_y + 5),
                f"F{frame + 1} halo={residual_count} cyan={cyan_after_count}",
                fill=(255, 255, 255, 255),
                font=FONT,
            )
            clip_payload[clip].append({
                "frame": frame,
                "foregroundPixelsAudited": int(foreground.sum()),
                "residualMagentaHaloPixels": residual_count,
                "cyanPhotophorePixelsBefore": cyan_before_count,
                "cyanPhotophorePixelsAfter": cyan_after_count,
                "changedCyanPhotophorePixels": changed_cyan,
            })
    save_png(canvas.convert("RGB"), output)
    if total_residual != 0:
        raise RuntimeError(f"neutral-background audit found {total_residual} magenta halo pixels")
    if total_changed_cyan != 0 or total_cyan_after < total_cyan_before:
        raise RuntimeError({
            "changedCyanPhotophorePixels": total_changed_cyan,
            "cyanBefore": total_cyan_before,
            "cyanAfter": total_cyan_after,
        })
    return {
        "schema": 1,
        "profileId": PROFILE_ID,
        "reviewedAt": REVIEW_DATE,
        "backgroundRgb": list(neutral[:3]),
        "method": (
            "Each final exact-magenta cell is alpha-matted without resampling its source pixels, "
            "composited over uniform neutral gray for visual inspection, and independently tested "
            "with a looser low-luminance magenta hue gate."
        ),
        "auditThreshold": spill_contract()["independentNeutralBackgroundAuditThreshold"],
        "clips": clip_payload,
        "summary": {
            "posesAudited": 32,
            "residualMagentaHaloPixels": total_residual,
            "cyanPhotophorePixelsBefore": total_cyan_before,
            "cyanPhotophorePixelsAfter": total_cyan_after,
            "restoredCyanEdgePixels": total_cyan_after - total_cyan_before,
            "changedCyanPhotophorePixels": total_changed_cyan,
            "cyanPhotophoresPreserved": True,
            "naturalPigmentsChangedOutsideDeclaredSpill": 0,
        },
        "accepted": False,
...
def draw_rigid_measurement_review(
    all_poses: dict[str, list[ExtractedPose]],
    output: Path,
) -> None:
    """Show the two authored anatomical-axis measurements used for every clip."""
    tile_w, tile_h, label_w = 720, 330, 110
    canvas = Image.new(
        "RGBA",
        (label_w + 2 * tile_w, len(CLIPS) * tile_h),
        (123, 128, 132, 255),
    )
    draw = ImageDraw.Draw(canvas)
    for clip_index, clip in enumerate(CLIPS):
        ...
        draw.text(
            (10, clip_index * tile_h + 14),
            f"{clip}\nrigid axis\n2 reviewed poses",
            fill=(255, 255, 255, 255),
            font=FONT,
        )
        for measurement_index, item in enumerate(RIGID_BODY_MEASUREMENTS[clip]):
            pose = all_poses[clip][int(item["frame"])]
            source = Image.open(pose.extracted_path).convert("RGBA")
            scale = min(
                (tile_w - 32) / source.width,
                (tile_h - 62) / source.height,
                1.7,
            )
            ...
            tile_x = label_w + measurement_index * tile_w
            tile_y = clip_index * tile_h
            paste_x = tile_x + (tile_w - resized.width) // 2
            paste_y = tile_y + 38 + (tile_h - 48 - resized.height) // 2
            canvas.paste(resized, (paste_x, paste_y), resized)
            endpoints = item["endpoints"]
            scaled_endpoints = [
                (
                    paste_x + round(int(point[0]) * scale),
                    paste_y + round(int(point[1]) * scale),
                )
                for point in endpoints
            ]
            draw.line((*scaled_endpoints[0], *scaled_endpoints[1]), fill=(255, 226, 48, 255), width=4)
            for point in scaled_endpoints:
                draw.ellipse(
                    (point[0] - 6, point[1] - 6, point[0] + 6, point[1] + 6),
                    outline=(60, 255, 120, 255),
                    width=3,
                )
            length = math.hypot(
                int(endpoints[1][0]) - int(endpoints[0][0]),
                int(endpoints[1][1]) - int(endpoints[0][1]),
            )
            draw.text(
                (tile_x + 8, tile_y + 8),
                f"F{pose.frame + 1} body-axis={length:.3f}px / RIGHT",
                fill=(255, 255, 255, 255),
                font=FONT,
            )
            draw.rectangle(
...
def draw_attack_action_review(image: Image.Image, reviews: list[dict[str, object]], output: Path) -> None:
    ...
    draw.text(
        (12, 836),
        "Manual review: one original right-facing Korari Stalker; wedge head, single jaw, four limbs, cyan flank marks and flexible tail remain coherent through the pounce-bite cycle.",
        fill=(255, 255, 255, 255),
        font=FONT,
    )
    save_png(canvas.convert("RGB"), output)
    ...
    return {
        ...
        "profiles": {
            PROFILE_ID: {
                ...
                "sourceScaleByClip": {clip: round(factors[clip], 9) for clip in CLIPS},
                "clips": clip_payload,
                "validation": {
                    "requiredAnchors": 32,
                    "reviewedAnchors": 32,
...
def build_scale_fragment(
    ...
    factors = {clip: float(scale["sourceScaleByClip"][clip]) for clip in CLIPS}
    ...
    for clip in CLIPS:
        ...
        measurements[clip] = {
            ...
            "appliedUniformClipScale": round(factors[clip], 9),
    ...
    return {
        ...
        "profiles": {
            PROFILE_ID: {
                "status": "measured-per-clip-uniform-scale-evidence-art-not-accepted",
                "reviewer": REVIEWER,
                "reviewedAt": REVIEW_DATE,
                "baselineClip": "idle",
                "method": (
                    "Two manual measurements per clip use the same rigid anatomical axis from the "
                    "posterior croup/tail-base contour to the anterior shoulder/neck junction. The "
                    "idle median is the baseline. One uniform x/y factor is applied to each whole "
                    "clip; no per-pose or non-uniform stretch is permitted. Silhouette height remains "
                    "pose-sensitive (leg flexion, head drop and collapse) and is reported only as a "
                    "composition diagnostic, never as physical body size."
                ),
                "scaleDerivation": scale,
                "measurements": measurements,
                "maximumIntegerRoundingAspectErrorPercent": round(max_rounding_error, 6),
                "decisions": {
                    clip: "apply one reviewed uniform clip factor from the rigid body-axis median" for clip in CLIPS
                },
                "sourceScaleByClip": {clip: round(factors[clip], 9) for clip in CLIPS},
                "factorsAppliedToActive": {clip: round(factors[clip], 9) for clip in CLIPS},
...
def build_visual_review() -> dict[str, object]:
    return {
        ...
        "reviewedAt": REVIEW_DATE,
        "basis": (
            "Manual visual inspection of the four differently sized raw boards, all 32 recovered "
            "poses, the exact-magenta recompositions, neutral-background despill audit, technical "
            "overlays, rigid-body measurement sheet and root-registered playbacks."
        ),
        "identity": {
            "sameKorariStalker": {
                ...
                "posesReviewed": 32,
                "note": (
                    "All poses remain recognizably the same original low forest quadruped: wedge "
                    "head with natural lateral eye/sensors, single jaw, four supple limbs, charcoal/"
                    "moss bark-pattern hide, sparse cyan flank photophores and one flexible tail."
                ),
            },
            "hide": "charcoal and moss organic bark-pattern hide retained across 32/32 poses",
            "photophores": "sparse dim cyan flank marks remain visible and are protected by despill invariants",
            "limbs": "four distinct animal limbs remain readable and physically grounded",
            "tail": "one long flexible non-bladed balance tail remains attached in every pose",
            "head": "one wedge head, one natural jaw and one lateral eye/sensor set remain invariant",
            "anatomy": (
                "project-original fauna only; no xenomorph dome, inner jaw, dorsal tubes, acid, "
                "Yautja armor, mask, dreadlocks, technology, extra limb or borrowed-franchise cue"
            ),
        },
        "direction": {
            "reviewedPoses": 32,
            ...
            "result": "pass",
            "deathQualification": (
                "Recoil and collapsed terminal poses preserve the right-facing wedge-head axis "
                "while the body settles laterally; no left-facing or camera-facing reversal occurs."
            ),
        },
        "motion": {
            "idle": "Eight distinct grounded breath, sensor-focus, tail-counterbalance and subtle photophore phases; loop closure is visually plausible.",
            "move": "Eight distinct low quadruped stalk/run phases with alternating contacts, supple spine and tail counterbalance.",
            "attack": {
                "projectOriginalFaunaOnly": True,
                "singleNaturalJaw": True,
                "lockedChronology": [
                    "compress",
                    "fix target right",
                    "coil hindquarters",
                    "short pounce",
                    "single-jaw bite and foreclaw contact",
                    "follow-through",
                    "land",
                    "guarded recovery"
                ],
                "pounceBiteCore": "frames 4-6 read as pounce, open single-jaw bite/contact and follow-through",
                "chronologyPass": True,
            },
            "death": {
        ...
        "consistency": {
            ...
            "observedDrift": [
                "individual bark-pattern plates and highlights vary between generated poses",
                "minor sensor-frill and cyan-photophore spacing is not pixel-identical across clips",
                "tail curvature varies naturally with the authored motion",
            ],
        },
        "technicalCorrection": {
            ...
            "neutralBackgroundResidualHaloPixels": 0,
            "changedOriginalCyanPhotophorePixels": 0,
            "evidence": [
                rel(SOURCE_QA_PATH),
                rel(DERIVED_PROVENANCE_PATH),
                rel(NEUTRAL_AUDIT_PATH),
            ],
        },
        "defects": [
            {
                ...
                "status": "open",
                "impact": (
                    "Broad Korari identity is consistent, but individual hide plates, sensor-frill "
                    "details and photophore spacing are not pixel-identical."
                ),
            },
            {
                "id": "post-recomposition-runtime-pending",
                "severity": "integration",
                "status": "open-by-scope",
                "impact": "The candidates remain unnormalized, unaccepted and outside runtime by design.",
            },
        ],
        "accepted": False,
        "runtimeIntegrated": False,
...
def build_derived_provenance(
    ...
    contact_path: Path,
    neutral_audit_image_path: Path,
    rigid_review_path: Path,
    ...
    return {
        ...
        "algorithm": {
            ...
            "poseRecovery": (
                "whole-board connected components recover complete poses that cross dynamically "
                "sized nominal source cells; eight primaries are ordered 4+4 by row and centroid x"
            ),
            "scale": scale,
            "resampling": "one reviewed uniform x/y factor per clip, premultiplied-alpha LANCZOS, no per-pose factor and no non-uniform stretch",
        ...
        "profileEvidence": {
            ...
            "neutralBackgroundDespillAudit": image_record(neutral_audit_image_path),
            "neutralBackgroundAuditData": file_record(NEUTRAL_AUDIT_PATH),
            "rigidBodyMeasurementReview": image_record(rigid_review_path),
...
def build_source_qa(
    ...
    for clip in CLIPS:
        ...
        selected_sources.append({
            ...
            "uniformClipScaleRecompositionPath": rel(derived_paths[clip]),
            "uniformClipScaleRecompositionSha256": sha256_file(derived_paths[clip]),
            ...
            "uniformClipScale": scale["sourceScaleByClip"][clip],
        ...
        clip_checks.append({
            ...
            "chronology": {
                "idle": "grounded breathing, ear and weight-shift loop",
                "move": "eight distinct natural quadruped locomotion phases",
                "attack": "compress, target lock, coil, pounce, single-jaw bite/contact, follow-through, land and guarded recovery",
                "death": "irreversible lateral collapse to motionless terminal pose",
            }[clip],
        })
    ...
    return {
        ...
        "method": {
            ...
            "recomposition": (
                "Each complete RGBA pose is uniformly resized by one x=y factor shared only "
                "within its clip and placed into an exact 1774x887 4x2 opaque-magenta board. Body-mass "
                "landmarks share the target cell centre root and stable support line."
            ),
            "scale": (
                "One common rigid croup-to-shoulder/neck target length is clamped once by the "
                "strictest all-pose fit constraint. Each clip receives one target/median x=y "
                "factor; no per-pose factor or non-uniform anatomy stretch is used."
            ),
            "despill": spill_contract(),
            "visual": (
                ...
                "contact sheet, neutral-background halo audit, rigid-measurement overlay and "
                "dedicated pounce-bite chronology sheet."
            ),
        ...
        "referenceEvidence": {
            ...
            "classification": "PROJECT_ORIGINAL",
            "canonExact": False,
        },
        "sharedScale": scale,
        ...
        "neutralBackgroundAudit": file_record(NEUTRAL_AUDIT_PATH),
        "visualEvidence": [
        ...
        "summary": {
            ...
            **validation,
            ...
            "artReviewDefectsOpen": 1,
            "integrationDefectsOpen": 1,
            "readOnlyContractDefectsOpen": 0,
            "artAccepted": False,
            "runtimeIntegrated": False,
            "canonExact": False,
        },
...
def build_event(
    ...
    actual_prompt_path = ACTUAL_PROMPT_DIR / f"{CANDIDATE_TAG[clip]}.txt"
    if not actual_prompt_path.is_file():
        raise RuntimeError(f"missing persisted actual ImageGen prompt for {clip}")
    actual_prompt_text = actual_prompt_path.read_bytes().decode("utf-8")
    actual_prompt_hash = sha256_bytes(
        json_stringify_prompt(actual_prompt_text).encode("utf-8")
    )
    actual_prompt_file_hash = sha256_file(actual_prompt_path)
    attack_qualification = None
    if clip == "attack":
        attack_qualification = (
            "Original right-facing Korari compresses, fixes target, coils, pounces, makes "
            "single-jaw/foreclaw contact, follows through, lands and recovers guarded."
        )
    despill_summary = {
        "spillThreshold": spill_contract(),
        "spillPixelsBefore": sum(
        ...
        "cyanPhotophorePixelsBefore": sum(
            int(pose["despill"]["cyanPhotophorePixelsBefore"]) for pose in review
        ),
        "cyanPhotophorePixelsAfter": sum(
            int(pose["despill"]["cyanPhotophorePixelsAfter"]) for pose in review
        ),
        "changedCyanPhotophorePixels": sum(
            int(pose["despill"]["changedCyanPhotophorePixels"]) for pose in review
        ),
    ...
    return {
        ...
        "actor": REVIEWER,
        "actualGenerationPromptAvailable": True,
        "actualPromptText": actual_prompt_text,
        "actualPromptPath": rel(actual_prompt_path),
        "actualPromptFileSha256": actual_prompt_file_hash,
        "actualPromptSha256": actual_prompt_hash,
        "currentQueuePromptSnapshotPath": rel(prompt_path),
        "currentQueuePromptFileBytesSha256": prompt_file_hash,
        "promptSha256": actual_prompt_hash,
        ...
        "actualPromptMatchesQueueContract": (
            actual_prompt_hash == queue_clip["promptSha256"]
        ),
        "promptProvenanceNote": (
            "The exact pre-call ImageGen prompt is preserved under the worklot-004 "
            "prompt directory. The current queue snapshot remains separate contract provenance."
        ),
        ...
        "sourceRepairs": [
            {
                "kind": (
                    "proven-magenta-component-extraction-and-per-clip-uniform-"
                    "scale-recomposition"
                ),
                "result": (
                    "Eight complete poses recovered across raw cell boundaries, uniformly "
                    "scaled with one reviewed uniform factor for this complete clip, root/support aligned, and recomposed "
                ...
                "uniformClipScale": round(scale_factor, 9),
            ...
            },
            {
                ...
                "result": (
                    ...
                    "and exact #FF00FF background were unchanged; independent neutral-gray "
                    "halo audit is zero and original cyan photophore pixels are unchanged."
                ),
                "neutralBackgroundAuditPath": rel(NEUTRAL_AUDIT_PATH),
                "neutralBackgroundAuditSha256": sha256_file(NEUTRAL_AUDIT_PATH),
                **despill_summary,
            },
        ],
        ...
        "qa": {
            ...
            "neutralBackgroundResidualHaloPixels": 0,
            "changedOriginalCyanPhotophorePixels": despill_summary[
                "changedCyanPhotophorePixels"
            ],
            "technicalReview": (
...
def main() -> None:
    ...
    scale_by_clip = {
        clip: float(scale["sourceScaleByClip"][clip]) for clip in CLIPS
    }
    boards: dict[str, np.ndarray] = {}
    reviews: dict[str, list[dict[str, object]]] = {}
    ...
    playback_paths: dict[str, Path] = {}
    for clip in CLIPS:
        guard_disk(f"{clip} rigid-calibrated uniform-scale recomposition")
        board, pose_reviews = compose_clip(clip, all_poses[clip], scale_by_clip[clip])
        derived_path = DERIVED_DIR / f"{CANDIDATE_TAG[clip]}-rigid-calibrated-recomposed.png"
    ...
    neutral_audit_image_path = QA_DIR / "neutral-background-despill-audit.png"
    rigid_review_path = QA_DIR / "rigid-body-measurement-review.png"
    attack_review_path = QA_DIR / "attack-pounce-bite-review.png"
    draw_contact(active_images, reviews, contact_path)
    neutral_audit = draw_neutral_background_audit(
        active_images,
        reviews,
        neutral_audit_image_path,
    )
    draw_rigid_measurement_review(all_poses, rigid_review_path)
    ...
    stage_summary = {
        ...
        "sourceScaleByClip": scale_by_clip,
        "fitLimitingPoseByClip": scale["fitLimitingPoseByClip"],
        ...
        "neutralBackgroundAudit": neutral_audit["summary"],
        "derivedSha256ByClip": {
    ...
    neutral_audit["imageEvidence"] = image_record(neutral_audit_image_path)
    write_json(NEUTRAL_AUDIT_PATH, neutral_audit)
    ...
    anchor_fragment = build_anchor_fragment(reviews, scale_by_clip)
    ...
    event_validation = {
        ...
        "requiredClips": 4,
        "issues": [],
        "getJobStatusSimulation": {
            "status": "generated",
            "generatedClips": 4,
            "recoveredPromptGaps": 0,
            "requiredClips": 4,
            "issues": [],
            "readOnly": True,
            "stateWrites": 0
    ...
    receipts = {
        ...
        "provider": "OpenAI ImageGen",
        "referenceLockSha256": queue_profile["referenceLockSha256"],
        "actualGenerationPromptsAvailable": True,
        "promptProvenance": (
            "Exact pre-call ImageGen prompts and separate current queue contract snapshots "
            "are both retained; their hashes are not conflated."
        ),
        "receipts": [{
            "clipId": clip,
            "actualPromptPath": rel(
                ACTUAL_PROMPT_DIR / f"{CANDIDATE_TAG[clip]}.txt"
            ),
            "actualPromptFileSha256": sha256_file(
                ACTUAL_PROMPT_DIR / f"{CANDIDATE_TAG[clip]}.txt"
            ),
            "actualPromptSha256": sha256_bytes(
                json_stringify_prompt(
                    (
                        ACTUAL_PROMPT_DIR / f"{CANDIDATE_TAG[clip]}.txt"
            ...
            "uniformClipScaleDerivative": file_record(derived_paths[clip]),
            "uniformClipScale": round(scale_by_clip[clip], 9),
            "despill": {
                "spillPixelsBefore": sum(
                    int(pose["despill"]["spillPixelsBefore"])
                    for pose in reviews[clip]
    ...
    manifest_paths = [
        ...
        *(ACTUAL_PROMPT_DIR / f"{tag}.txt" for tag in CANDIDATE_TAG.values()),
        *asset_outputs,
        *prompt_paths.values(),
        *event_paths.values(),
    ]
