"""Deterministic, profile-local source processing for enemy-049.

The four immutable OpenAI raw captures are the only art inputs.  All writes are
limited to the enemy-049 frame directory and this standalone review directory.
...
PROFILE_ID = "enemy-049-wild-boar-host"
BATCH_ID = "batch-004"
CLIPS = ("idle", "move", "attack", "death")
SELECTED = {
    "idle": "idle-r3-imagegen.png",
    "move": "move-r2-imagegen.png",
...
RIGID_BODY_MEASUREMENTS = {
    "idle": [
        {"frame": 0, "endpoints": [[55, 80], [235, 75]]},
        {"frame": 1, "endpoints": [[45, 80], [225, 70]]},
    ],
    "move": [
        {"frame": 0, "endpoints": [[40, 70], [220, 65]]},
        {"frame": 1, "endpoints": [[45, 75], [220, 70]]},
    ],
    "attack": [
        {"frame": 0, "endpoints": [[40, 85], [190, 75]]},
        {"frame": 1, "endpoints": [[35, 80], [180, 70]]},
    ],
    "death": [
        {"frame": 0, "endpoints": [[45, 90], [220, 75]]},
        {"frame": 1, "endpoints": [[50, 85], [210, 80]]},
    ],
}
...
ACTUAL_PROMPT_DIR = (
    ROOT / "docs/references/v66-worklot-003-prompts" / PROFILE_ID
)
...
REFERENCE_LOCK_PATH = ROOT / "docs/references/V66_WORKLOT_003_WILD_BOAR_HOST_REFERENCE.json"
...
def strict_edge_spill_colour_mask(array: np.ndarray) -> np.ndarray:
    """V64-compatible proof of vivid magenta contamination, never russet/brown."""
    red = array[:, :, 0].astype(np.int16)
    green = array[:, :, 1].astype(np.int16)
    blue = array[:, :, 2].astype(np.int16)
    return (
...
def residual_matte_hue_mask(array: np.ndarray) -> np.ndarray:
    """Dark anti-aliased matte hue; close R/B excludes brown/russet bristles."""
    red = array[:, :, 0].astype(np.int16)
    green = array[:, :, 1].astype(np.int16)
    blue = array[:, :, 2].astype(np.int16)
    return (
...
def spill_contract() -> dict[str, object]:
    return {
        ...
        "residualMatteHueThreshold": {
            ...
            "purpose": "remove only edge pixels retaining matte-like R/B parity; russet/brown with low blue is excluded",
        },
...
def despill_cell(cell: np.ndarray) -> tuple[np.ndarray, dict[str, object]]:
    ...
    spill_before = strict_before | residual_before | delivery_before
    source_mask = (foreground_before & ~strict_edge_spill_colour_mask(before)
                   & ~residual_matte_hue_mask(before) & ~delivery_violet_matte_mask(before))
    output = before.copy()
    ...
    return output, {
        ...
        "sourceAuthoredRussetPreserved": True,
...
def determine_shared_scale(all_poses: dict[str, list[ExtractedPose]]) -> dict[str, object]:
    ...
    for clip in CLIPS:
        ...
        for item in RIGID_BODY_MEASUREMENTS[clip]:
            ...
            rigid[clip].append({
                "clip": clip,
                "frame": frame,
                "landmark": "posterior croup at tail-base contour to anterior shoulder/neck junction",
                "endpoints": endpoints,
    ...
    baseline_factor = min(idle_factor, float(fit_limits["idle"]["factor"]), 1.0)
    requested = {
        clip: baseline_factor * medians["idle"] / medians[clip] for clip in CLIPS
    }
    applied = {
        clip: min(requested[clip], float(fit_limits[clip]["factor"])) for clip in CLIPS
    }
    ...
    return {
        ...
        "rigidLandmark": "posterior croup at tail-base contour to anterior shoulder/neck junction",
        ...
        "selection": "idle baseline is fit-clamped; each other clip uses idle rigid-median / clip rigid-median and is independently fit-clamped; one uniform factor per complete clip, never per pose",
    }
...
def resize_pose_rgba(pose: ExtractedPose, factor: float) -> tuple[Image.Image, dict[str, object]]:
    ...
    # into the preserved boar silhouette. Width and height share one factor.
...
def draw_attack_action_review(image: Image.Image, reviews: list[dict[str, object]], output: Path) -> None:
    ...
    draw.text(
        (12, 836),
        "Manual review: one natural right-facing boar; tusks, four legs and low body remain coherent through the full head-drive cycle.",
        fill=(255, 255, 255, 255),
        font=FONT,
    )
    save_png(canvas.convert("RGB"), output)
...
def build_visual_review() -> dict[str, object]:
    return {
        ...
        "identity": {
            "sameNaturalCompactWildBoar": {
                "result": "pass-broad-identity",
                "posesReviewed": 32,
                "note": (
                    "All poses remain recognizably the same compact adult wild boar: low "
                    "shoulder mass, long snout, two tusks, four mammalian legs and a short tail."
                ),
            },
            "earthyBristles": "dark russet-brown organic coat retained across 32/32 poses",
            "tusks": "paired pale tusks remain readable without becoming oversized fantasy weapons",
            "hooves": "four cloven-hoof legs remain mammalian and grounded",
            "tail": "short natural boar tail retained; no blade or xenomorph tail",
            "restrainedInjury": "death may show restrained blood; no dismemberment or mutation",
            "anatomy": (
                "natural quadruped boar anatomy only; no biomechanical carapace, dorsal tubes, "
                "inner jaw, armor, humanoid torso or fantasy-monster appendage"
            ),
        },
        "direction": {
            "reviewedPoses": 32,
            ...
            "result": "pass",
            "deathQualification": (
                "Collapsed terminal poses preserve the right-facing snout/tusk axis while the "
                "body settles laterally; no camera-facing belly-up roll is used."
            ),
        },
        "motion": {
            "idle": "Eight distinct grounded breathing, ear and weight-shift poses; loop closure is visually plausible.",
            "move": "Eight distinct natural quadruped walk/run phases with alternating hooves.",
            "attack": {
                "naturalBoarOnly": True,
                "noInnerJawOrAlienMutation": True,
                "tuskHeadDriveCore": "frames 4-6 read as a low forward head/tusk shove",
                "chronologyPass": True,
            },
            "death": {
        ...
        "consistency": {
            ...
            "observedDrift": [
                "individual bristle clumps and highlights vary between generated poses",
                "minor tusk curvature and ear-fold details are not pixel-identical across clips",
                "restrained injury marks vary during the authored death progression",
            ],
        },
        "technicalCorrection": {
        ...
        "defects": [
            {
                ...
                "status": "open",
                "impact": (
                    "Broad natural-boar identity is consistent, but individual bristle, "
                    "ear and tusk micro-details are not pixel-identical."
                ),
            },
            {
    ...
    for clip in CLIPS:
        ...
        clip_checks.append({
            ...
            "chronology": {
                ...
                "attack": "low head/tusk drive, contact, follow-through and recovery",
    ...
    return {
        ...
        "method": {
            ...
            "visual": (
                ...
                "contact sheet and a dedicated tusk/head-drive action sheet."
            ),
...
def build_event(
    ...
    if clip == "attack":
        attack_qualification = (
            "Natural right-facing boar head/tusk drive with grounded foreleg brace, "
            "contact, follow-through and recovery; no alien mutation or fantasy weapon."
        )
    despill_summary = {
        "spillThreshold": spill_contract(),
        "spillPixelsBefore": sum(
    ...
    return {
        ...
        "promptProvenanceNote": (
            "The exact pre-call ImageGen prompt is preserved under the worklot-003 "
...
def main() -> None:
    ...
    attack_review_path = QA_DIR / "attack-tusk-head-drive-review.png"
    ...
    event_validation = {
        ...
        "jobStatus": "existing-candidates-processed-art-not-accepted",
        "processedClips": 4,
        "requiredClips": 4,
        "issues": [],
        "events": [{
