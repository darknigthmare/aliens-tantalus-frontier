...
try {
  ...
  await command('Page.navigate', { url: `${appUrl}${appUrl.includes('?') ? '&' : '?'}qa=v64-${Date.now()}` });
  ...
  requireThat(shell.title.includes('v64') && shell.release === '64.0.0' && shell.schema === 52, `Version publique incorrecte: ${JSON.stringify(shell)}`);
  ...
  requireThat(missionStart.animationRuntime?.sheets === 195 && missionStart.animationRuntime.runtimeReady === 195 && missionStart.animationRuntime.invalid.length === 0, `Contrat animation runtime V64 incomplet: ${JSON.stringify(missionStart.animationRuntime)}`);
  ...
  await command('Page.reload', { ignoreCache: false });
  await wait(900);
  await waitFor(`Boolean(globalThis.__ATF_V51__ && globalThis.__ATF_V61__ && globalThis.__ATF_V62__ && globalThis.__ATF_V51__.saveSystem.data.release === '64.0.0' && !document.querySelector('#boot'))`, 'Boot hors-ligne V64 impossible', 20000);
  await evaluate(`(() => { globalThis.__ATF_V61__.titleScreen.openMenu(); globalThis.__ATF_V61__.titleScreen.continueGame(); return true; })()`);
  ...
  requireThat(offline.release === '64.0.0' && offline.controlled && offline.appVisible && !offline.overlay && criticalOfflineFailures.length === 0, `PWA hors-ligne V64 incomplète: ${JSON.stringify({ offline, criticalOfflineFailures })}`);
