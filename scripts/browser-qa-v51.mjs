...
try {
  ...
  await command('Page.navigate', { url: `${appUrl}${appUrl.includes('?') ? '&' : '?'}qa=v51-${Date.now()}` });
  ...
  requireThat(shell.title.includes('v51') && shell.release === '51.0.0' && shell.schema === 51, `Version publique incorrecte: ${JSON.stringify(shell)}`);
  requireThat(shell.appVisible && !shell.overlay && shell.worlds === 64 && shell.campaigns === 436 && shell.editorTools === 13, `Shell v51 incomplet: ${JSON.stringify(shell)}`);
  report.shell = shell;
  report.checkpoints.push('boot-v51');
  ...
  report.screenshots.push(await capture('alien-tantalus-v51-mission-desktop.png'));
  ...
  const resumeBefore = await evaluate(`(() => {
    ...
    return { victimId: victim.id, dropId: drop.id, schema: stored.schema, identity: stored.identity, checkpoint: stored.checkpoint, health: stored.player.health };
  })()`);
  requireThat(resumeBefore.schema === 1 && resumeBefore.identity && resumeBefore.checkpoint.id === 'power', `Snapshot natif non persisté avant reload: ${JSON.stringify(resumeBefore)}`);
  ...
  requireThat(resumeAfter.result.applied && resumeAfter.state.checkpoint.id === 'power' && resumeAfter.state.player.health === persistedResume.player.health, `Checkpoint/joueur non repris: ${JSON.stringify({ persistedResume, resumeAfter })}`);
  ...
  await evaluate(`(() => {
    ...
    api.saveSystem.data.hub.activeCrisis = { id: 'qa-v51-xeno-crisis', kind: 'xenomorph', count: 1, deck: 0, roomId: 'bridge', resolved: false };
  ...
  report.screenshots.push(await capture('alien-tantalus-v51-hub-crisis-desktop.png'));
  ...
  await command('Page.reload', { ignoreCache: true });
  await wait(900);
  await waitFor(`Boolean(globalThis.__ATF_V51__ && !document.querySelector('#boot') && !document.querySelector('#app').hidden)`, 'Reload mobile v51 incomplet', 20000);
  ...
  requireThat(mobile.width === 390 && mobile.activePanel === 'hub' && mobile.canvasWidth <= 390 && mobile.canvasWidth >= 300 && mobile.controlsDisplay !== 'none' && mobile.controlCount === 6 && mobile.hubRunning, `Runtime mobile incomplet: ${JSON.stringify(mobile)}`);
  ...
  report.screenshots.push(await capture('alien-tantalus-v51-hub-mobile.png'));
  ...
  await command('Page.reload', { ignoreCache: false });
  await wait(900);
  await waitFor(`Boolean(globalThis.__ATF_V51__ && globalThis.__ATF_V51__.saveSystem.data.release === '51.0.0' && !document.querySelector('#boot'))`, 'Boot hors-ligne v51 impossible', 20000);
